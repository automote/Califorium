...
public class NetworkConfig {
	 ...
	 */
	public class Keys {
		public static final String COAP_PORT = "5683";
	 ...
	 *
	 * @param coapPort the key
	 * @return the int
	 */
	public int getInt(String coapPort) {
		//LOGGER.info("net config " + key);

		String value = properties.getProperty(coapPort);
		//LOGGER.info("net config_value " + value);
		if (value != null) {
			try {
				return Integer.parseInt(value);
			} catch (NumberFormatException e) {
				LOGGER.log(Level.WARNING, "Could not convert property \"" + coapPort + "\" with value \"" + value + "\" to integer", e);
			}
		} else {
			LOGGER.warning("Property \"" + coapPort + "\" is undefined");
		}
		return 0;
	}
